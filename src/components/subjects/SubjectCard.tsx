...
export function SubjectCard({ subject, onOpen }: SubjectCardProps) {
  ...
  return (
    <button
      className="glass relative w-full rounded-[28px] border-t-4 p-4 text-left transition active:scale-[0.99]"
    ...
    >
      ...
      <div className="pr-6">
        ...
        {subject.fullName ? <p className="mt-1 line-clamp-2 text-sm text-[var(--text-secondary)]">{subject.fullName}</p> : null}
      </div>
      <div className="mt-4 flex items-end justify-between gap-3">
        <div>
          <p className="text-3xl font-bold">{formatGrade(grade.average)}</p>
          <p className="text-xs uppercase tracking-wide text-[var(--text-secondary)]">{grade.isPartial ? 'média parcial' : 'média final'}</p>
        </div>
        <Badge tone={tone}>{statusLabel(grade.status)}</Badge>
      </div>
      <div className="mt-4 flex items-center gap-2 text-sm text-[var(--text-secondary)]">
        <Clock3 size={16} />
        <span>{next ? `${next.name} — ${formatCountdown(next.date)}` : 'Sem avaliações datadas'}</span>
      </div>
      {checklist.length ? (
